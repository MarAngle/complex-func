...
let utils = {
  ...
  updateList: function(targetlist, originlist, option = {}) {
    ...
    if (!option.update) {
      ...
      if (this.getType(option.update) == 'function') {
        updateType = 'function'
      }
    }
  ...
  */
  ...
  defineReactive: function(data, key, val, option = {}) {
  ...
  // 监控对象属性
  defineWatch: function(data, key, option = {}) {
    let type = typeof data
    if (type !== 'object') {
  ...
  // 创建响应式数据
  // 存在get/set时writable属性的设置不生效
  defineDeepReactive: function(data, key, val, option = {}) {
    const property = Object.getOwnPropertyDescriptor(data, key)
    if (property && property.configurable === false) {
      this.printMsg('defineReactive时data配置中configurable不能为false')
      return false
  ...
  // 监控对象属性
  defineDeepWatch: function(data, key, option = {}) {
    let type = typeof data
    if (type !== 'object') {
      this.printMsg('defineWatch中data只能接收object')
      return false
  ...
  findTargetInStr: function(str, target, option = {}) {
    if (str && target) {
      str = str.toString()
  ...
  // 获取指定字符串在目标字符串中的位置数组
  findTargetInStrNext: function(str, target, limitNum, list = [], index = 0) {
  ...
  runFunction: function(func, args, callback) {
    let mainRes = {
      status: 'fail',
      code: ''
    }
